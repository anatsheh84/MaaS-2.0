...
if oc get secret llamastack-s3 -n "$RAG_NS" &>/dev/null; then
  warn "llamastack-s3 already exists in $RAG_NS — skipping"
else
  # Generate a unique bucket name using the cluster infraID
  S3_BUCKET="llamastack-files-${INFRA_ID}"
  info "Creating S3 bucket: $S3_BUCKET in $REGION"
  ...
  if command -v aws &>/dev/null; then
    ...
    warn "    --create-bucket-configuration LocationConstraint=$REGION"
  fi

  oc create secret generic llamastack-s3 -n "$RAG_NS" \
    --from-literal=bucket-name="$S3_BUCKET" \
    --from-literal=region="$REGION" \
    --from-literal=aws-access-key-id="$AWS_ACCESS_KEY_ID" \
    --from-literal=aws-secret-access-key="$AWS_SECRET_ACCESS_KEY"
  success "llamastack-s3 secret created in $RAG_NS"
fi
